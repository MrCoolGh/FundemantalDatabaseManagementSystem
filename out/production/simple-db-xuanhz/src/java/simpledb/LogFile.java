    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                Set<HeapPageId> alreadyRolled = new HashSet<>();
                boolean CLRisWrote = false;
                while (true) {
                    try {
                        int rType = raf.readInt();
                        long rTid = raf.readLong();
                        if (rType == UPDATE_RECORD && rTid == tid.getId()) {
                            HeapPage pageBeforeImage = (HeapPage)readPageData(raf);
                            long currentPos = raf.getFilePointer();
                            if (!alreadyRolled.contains(pageBeforeImage.getId())) {
                                HeapFile rollBackFile = (HeapFile)Database.getCatalog().getDatabaseFile(pageBeforeImage.getId().getTableId());
                                rollBackFile.writePage(pageBeforeImage.getBeforeImage());
                                Page dirtyPage = Database.getBufferPool().bufferPool.get(pageBeforeImage.getId());
                                Database.getBufferPool().discardPage(pageBeforeImage.getId());
                                alreadyRolled.add(pageBeforeImage.getId());
                                raf.seek(raf.length());
                                raf.writeInt(CLR_RECORD);
                                raf.writeLong(tid.getId());
                                writePageData(raf, dirtyPage);
                                writePageData(raf, pageBeforeImage);
                                raf.writeLong(currentOffset);
                                force();
                                CLRisWrote = true;
                            }
//                            raf.seek(currentPos);
                            //skip after image
                            HeapPage pageAfterImage = (HeapPage)readPageData(raf);
                            // recover to the start: the before image of first update record.
                            // write CLR record
                            /*
                               record type
                               transaction id
    ...
    */
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                // some code goes here
                long originOffset = raf.getFilePointer();
                // first step find the last checkpoint
                Map<Long, Long> activeOffset = new HashMap<>();
                Map<Long, Long> commitOffset = new HashMap<>();
                ...
                if (checkPointOffset != -1) {
                    ...
                    for (int i = 0; i < numTrans; i++) {
                        long checkPointTid = raf.readLong();
                        long startOffset = raf.readLong();
                        activeOffset.put(checkPointTid, startOffset);
                    }
                }

                long beginOffset = (checkPointOffset == -1) ? 0 : checkPointOffset;
                ...
                while (true) {
                    try {
                        long rOffset = raf.getFilePointer();
                        int rType = raf.readInt();
                        long rTid = raf.readLong();
                        if (rType == ABORT_RECORD) {
                            activeOffset.remove(rTid);
                        } else if (rType == COMMIT_RECORD) {
                            activeOffset.remove(rTid);
                            commitOffset.put(rTid, rOffset);
                        } else if (rType == UPDATE_RECORD){
                            Page beforePage = readPageData(raf); // before
                            Page afterPage = readPageData(raf); // after
                            HeapFile file = (HeapFile)Database.getCatalog().getDatabaseFile(beforePage.getId().getTableId());
                            file.writePage(afterPage);
                        } else if (rType == BEGIN_RECORD) {
                            activeOffset.put(rTid, rOffset);
                        } else if (rType == CHECKPOINT_RECORD) {
                            int numTrans = raf.readInt();
                            for (int i = 0; i < numTrans; i++) {
                                long tid = raf.readLong();
                                long tStartOff = raf.readLong();
                            }
                        } else if (rType == CLR_RECORD){
                            //CLR
                            Page modifiedPage = readPageData(raf); // dirtied
                            Page originalPage = readPageData(raf); // undone
                            HeapFile file = (HeapFile)Database.getCatalog().getDatabaseFile(modifiedPage.getId().getTableId());
                            file.writePage(originalPage);
                        }
                        raf.readLong();
                    } catch (EOFException eofError) {
                        break;
                ...
                // 3.
                long earliestActiveOffset = Integer.MAX_VALUE;
                for (Long activeTrans: activeOffset.keySet()) {
                    if (activeOffset.get(activeTrans) < earliestActiveOffset) {
                        earliestActiveOffset = activeTrans;
                    }
                }
                long undoStopOffset = Math.min(earliestActiveOffset, checkPointOffset);
                raf.seek(raf.length() - LONG_SIZE);
                while (raf.getFilePointer() >= undoStopOffset && raf.getFilePointer() > 0) {
                    long startOffset = raf.readLong();
                    raf.seek(startOffset);
                    int rType = raf.readInt();
                    long rTid = raf.readLong();
                    if (rType == UPDATE_RECORD) {
                        Page before = readPageData(raf);
                        if (activeOffset.containsKey(rTid)) {
                            DbFile targetFile = Database.getCatalog().getDatabaseFile(before.getId().getTableId());
                            targetFile.writePage(before.getBeforeImage());
                        }
                        Page after = readPageData(raf);
                    } else if (rType == CHECKPOINT_RECORD) {
                        int numTrans = raf.readInt();
                        ...
                        Page dirtied = readPageData(raf);
                        Page undone = readPageData(raf);
                    }
    ...
    public void print() throws IOException {
        ...
        System.out.println("0: checkpoint record at transaction " + raf.readLong());
            ...
            try {
                ...
                switch (cpType) {
                    case BEGIN_RECORD:
                        ...
                        System.out.println((raf.getFilePointer() - (INT_SIZE + LONG_SIZE)) + ": RECORD TYPE BEGIN");
                    ...
                    case ABORT_RECORD:
                        ...
                        System.out.println((raf.getFilePointer() - (INT_SIZE + LONG_SIZE)) + ": RECORD TYPE ABORT");
                        System.out.println((raf.getFilePointer() - LONG_SIZE) + ": TID " + cpTid);
                        System.out.println(raf.getFilePointer() + ": RECORD START OFFSET: " + raf.readLong());
                        break;
                    case COMMIT_RECORD:
                        System.out.println(" (COMMIT)");
                        System.out.println((raf.getFilePointer() - (INT_SIZE + LONG_SIZE)) + ": RECORD TYPE COMMIT");
                        System.out.println((raf.getFilePointer() - LONG_SIZE) + ": TID " + cpTid);
                        System.out.println(raf.getFilePointer() + ": RECORD START OFFSET: " + raf.readLong());
                        break;
                    ...
                    case CHECKPOINT_RECORD:
                        ...
                        System.out.println((raf.getFilePointer() - (INT_SIZE + LONG_SIZE)) + ": RECORD TYPE CheckPoint");
                        System.out.println((raf.getFilePointer() - LONG_SIZE) + ": TID " + cpTid);
                    ...
                    case UPDATE_RECORD:
                        ...
                        System.out.println((raf.getFilePointer() - (INT_SIZE + LONG_SIZE)) + ": RECORD TYPE Update");
                        System.out.println((raf.getFilePointer() - LONG_SIZE) + ": TID " + cpTid);
                    ...
                    case CLR_RECORD:
                        ...
                        System.out.println((raf.getFilePointer() - (INT_SIZE + LONG_SIZE)) + ": RECORD TYPE CLR");
                        System.out.println((raf.getFilePointer() - LONG_SIZE) + ": TID " + cpTid);
                        HeapPage modifiedPage = (HeapPage) readPageData(raf);
                        HeapPage redoPage = (HeapPage) readPageData(raf);
                        System.out.println("table id:" + modifiedPage.getId().getTableId());
                        System.out.println("page number:" + modifiedPage.getId().getPageNumber());
                        System.out.println("before(modified data): " + Arrays.toString(modifiedPage.tuples));
                        System.out.println("after(redone data): " + Arrays.toString(redoPage.tuples));
                        System.out.println(raf.getFilePointer() + ": RECORD START OFFSET: " + raf.readLong());
                        break;
                }
                System.out.println();
